...
type Handler struct {
	cs  *ConsignmentService
	cha *CHAService
}

func NewHandler(cs *ConsignmentService, cha *CHAService) *Handler {
	return &Handler{cs: cs, cha: cha}
}
...
func (c *Handler) HandleCreateConsignment(w http.ResponseWriter, r *http.Request) {
...
func (c *Handler) HandleGetConsignments(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	...
	if stateStr := r.URL.Query().Get("state"); stateStr != "" {
		state := ConsignmentState(stateStr)
		filter.State = &state
	}
	if flowStr := r.URL.Query().Get("flow"); flowStr != "" {
		flow := ConsignmentFlow(flowStr)
		filter.Flow = &flow
	}
...
func (c *Handler) HandleInitializeConsignment(w http.ResponseWriter, r *http.Request) {
	authCtx := auth.GetAuthContext(r.Context())
	if authCtx == nil {
		http.Error(w, "Unauthorized", http.StatusUnauthorized)
		return
...
func (c *Handler) HandleGetConsignmentByID(w http.ResponseWriter, r *http.Request) {
